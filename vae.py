...
class Block(nn.Module):
    ...
    def __init__(self, in_ch, out_ch):
        ...
        self.relu = nn.LeakyReLU(negative_slope=0.2) # leaky ReLU 
...
class Encoder(nn.Module):
    ...
    def __init__(self, spatial_size=[64, 64], z_dim=256, chs=(1, 64, 128, 256)):
        ...
        #_h, _w = spatial_size[0]//2, spatial_size[1]//2
...
class Generator(nn.Module):
    ...
    def __init__(self, z_dim=256, chs=(256, 128, 64, 32), h=8, w=8):
        ...
        self.upconvs = nn.ModuleList(
            [nn.ConvTranspose2d(chs[i], chs[i], 2, 2) for i in range(len(chs) - 1)] # transpose convolution           
        )

        self.dec_blocks = nn.ModuleList(
            [Block(chs[i], chs[i + 1]) for i in range(len(chs) - 1)] # convolutional blocks           
        )
